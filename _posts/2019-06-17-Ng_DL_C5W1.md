...
- $\Gamma_u = \sigma(w_u \lbrack a^{<t-1>}, x^{<t-1>} \rbrack + b_u)$ （更新门）
- $\Gamma_f = \sigma(w_f \lbrack a^{<t-1>}, x^{<t-1>} \rbrack + b_f)$ （遗忘门）
- $\Gamma_o = \sigma(w_o \lbrack a^{<t-1>}, x^{<t-1>} \rbrack + b_o)$ （输出门）
