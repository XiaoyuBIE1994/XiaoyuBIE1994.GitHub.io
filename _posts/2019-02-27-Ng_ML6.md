...
SVM会使分界超平面离数据的距离最远，下图所示是二维情况下的一种形象化理解，当间隔最大时，由于 $\theta​$ 和平面垂直，所以数据投影在 $\theta​$ 上的距离最大，也就是 $p^{(i)}​$ 最大，而算法需要满足分类条件，所以所需要的 $||\theta||​$  就可以最小，对应的 $\frac{1}{2}\mathop{\min}_{\theta}\sum_{i=1}^m \theta_j^2 ​$
