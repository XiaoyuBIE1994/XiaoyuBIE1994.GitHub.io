...
&emsp;&emsp;&emsp;&emsp; $\delta^{(L)} = a^{(L)} - y^{(i)}$  
&emsp;&emsp;&emsp;&emsp; $\delta^{(l)} = ((\Theta^{(l)})^T \delta^{(l+1)}).*a(l).*(1-a(l))) \; for \; (i = L-1, L-2, ..., 2)$  
&emsp;&emsp;&emsp;&emsp; $\Delta_{ij}^{(l)} := \Delta_{ij}^{(l)} + a_j^{(l)}\delta_i^{(l+1)} \quad
